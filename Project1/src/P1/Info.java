...
public class Info {
	...
	/** The main executing method
		...
		@throws java.io.IOException  Exception for when the given URL cannot be read in properly
		*/
		...
		/** drinkSearch requests a search criteria from the user, 
		 *  hits the url with query (with helper method FileURLQuery),
		 *  converts the response into drink objects (with helper method), 
		 *  and adds unique drinks to the master drink list.
		@author	Ian M McCormick imccor455
		...
		*/
		...
		/** FileURLQuery takes in the search from the user,
		 *  hits the URL endpoint with the query,
		 *  returns the URL file.
		@author	Ian M McCormick imccor455
		...
		*/
		...
		/** getQueryResult modified version from ReadWeb.java to return a string version of the URL response.
		@author	Ian M McCormick imccor455
		@author Dr. Spiegel
		...
		*/
		...
		/** stringToJSONArray Takes in the full string response from the URL,
		 *  the string is converted to a JSONObject,
		 *  the list of drinks in the object are converted to a JSONArray.
		@author	Ian M McCormick imccor455
		@param drinkString The full string response from the URL containing a main "drinks" key,
				with a value containing a list of drinks.
		@return	drinkList A JSONArray version of the URL response containing the drinks inside "drinks".
		...
		*/
		...
		/** drinkArrayToList takes in the jsonarray version of the response,
		 * parses through the object to isolate individual drinks,
		 * drinks are created into "Drink" objects,
		 * and then added to a Drink List and returned.
		@author	Ian M McCormick imccor455
		@param drinkList The jsonarray version of the response containing found drinks.
		...
		*/
		...
		/** addDrinkToList takes in a drink and the master drink list,
		 *  verifies if the current drink exists in the master list,
		 *  if not, it adds the drink to the list.
		@author	Ian M McCormick imccor455
		@param drink The newly queried drink to be analyzed and potentially added to the master list
		...
		*/
		...
		/** printMasterList prints out all the drinks' name in the master list.
		 *  will print that not drinks were found if none were found yet.
		@author	Ian M McCormick imccor455
		@param masterList The main table/list containing all found queried drinks at the point of execution.
		*/
